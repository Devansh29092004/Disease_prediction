...
export async function runInference(
  ...
  try {
    ...
    const tensor = new onnx.Tensor('float32', new Float32Array(inputData), [1, inputData.length]);
    
    // Run inference with input name from model metadata
    console.log(`Starting model inference with input shape: [1, ${inputData.length}]`);
    ...
    console.log('First few values of output data:', Array.from(outputTensor.data as any).slice(0, 10));
    
    // Convert output data to numbers to avoid BigInt issues
    let outputArray: number[];
    
    if (outputTensor.data instanceof Float32Array || outputTensor.data instanceof Float64Array) {
